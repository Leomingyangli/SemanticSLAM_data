...
class preprocess():
    ...
    def generate(self, key='train'):
        ...
        for index in tqdm(range(self.nsplit)):
            ...
            semnatic_gt_poses = torch.zeros(self.num_steps-1, self.angles.shape[0], self.args.map_size, self.args.map_size, device=self.device)  #((L-1)*bs,nangles, mapsize,mapsize)
            semnatic_gt_poses[range(int(self.num_steps-1)) ,index_pose[:, 2], index_pose[:, 0], index_pose[:, 1]] = 1.0
